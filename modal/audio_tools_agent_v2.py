...
app = modal.App("ezway-audio-tools-v2")

# Keep Volume mount points completely absent from the image build.
# Cache environment variables are configured only after the Volume is mounted
# inside a running function/container.
...
def _configure_runtime_cache() -> None:
    """Point model caches into the mounted Volume after container startup."""
    model_root = Path(MODEL_ROOT)
    model_root.mkdir(parents=True, exist_ok=True)
    os.environ["TORCH_HOME"] = str(model_root / "torch")
    os.environ["HF_HOME"] = str(model_root / "hf")
