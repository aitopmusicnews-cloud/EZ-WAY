...
# Keep the same Modal app so the existing production endpoint name remains stable.
app = base.app

# TorchAudio 2.9+ delegates save() to TorchCodec. PyTorch/TorchCodec 0.13 supports
# current torch releases (>=2.11), so add it only to the GPU audio image.
patched_audio_image = (
    base.audio_image
    .uv_pip_install("torchcodec>=0.13,<0.14")
    .add_local_python_source("audio_tools_agent_v2")
)

patched_web_image = base.web_image.add_local_python_source("audio_tools_agent_v2")
...
def _run_demucs(source: Path, output_dir: Path, two_stem: bool) -> Path:
    ...
    if completed.returncode != 0:
        # Keep Modal exceptions small enough to return through the polling endpoint.
    ...
    def generate_synced_lyrics(self, payload: dict, file_base_url: str) -> dict:
        ...
        with tempfile.TemporaryDirectory(prefix="ezway-lyrics-") as temp_name:
            ...
            return {
                ...
                "job_id": job_id,
                "action": "lyrics",
                "engine": "demucs-htdemucs+torchcodec+faster-whisper-large-v3",
