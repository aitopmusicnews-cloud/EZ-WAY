    ...
    <div className="min-h-full bg-black text-white p-4 sm:p-6 lg:p-8">
      <div className="max-w-[1500px] mx-auto space-y-6">
        ...
        <section className="rounded-[2rem] border border-zinc-900 bg-zinc-950 p-5 sm:p-6">
          ...
          {metrics?.quality_trend?.length ? <div className="mt-5 rounded-3xl border border-zinc-800 bg-black p-4"><div className="flex items-center gap-2 mb-3"><BarChart3 className="w-4 h-4 text-orange-500" /><p className="text-xs font-black">Quality trend</p></div><div className="flex gap-2 overflow-x-auto pb-1">{metrics.quality_trend.map((point) => <div key={`${point.version}-${point.set_number}`} className="min-w-[120px] rounded-2xl border border-zinc-900 bg-zinc-950 p-3"><p className="text-[9px] text-zinc-600">v{point.version} · set {point.set_number}</p><p className="text-lg font-black mt-1">{point.winner_score == null ? '—' : asNumber(point.winner_score, 1)}</p><p className="text-[9px] text-zinc-600">winner</p></div>)}</div></div> : null}
        </section>
