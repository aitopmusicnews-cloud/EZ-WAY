...
@dataclass(frozen=True)
class ServiceConfig:
    # Cloud deployment: default host is 0.0.0.0, port 8080 (App Runner standard)
    # Local development: override with LYRIC_SERVICE_HOST=127.0.0.1 LYRIC_SERVICE_PORT=8765
    host: str = "0.0.0.0"
    port: int = 8080
    ...
    transcribe_bucket: str = ""  # Required for cloud transcription

    @classmethod
    def from_env(cls) -> "ServiceConfig":
        host = os.getenv("LYRIC_SERVICE_HOST", "0.0.0.0").strip() or "0.0.0.0"
        try:
            port = max(1, min(65535, int(os.getenv("LYRIC_SERVICE_PORT", "8080"))))
        except ValueError:
            port = 8080
        try:
        ...
        return cls(
            ...
            transcribe_bucket=os.getenv("TRANSCRIBE_S3_BUCKET", "").strip(),
        )
