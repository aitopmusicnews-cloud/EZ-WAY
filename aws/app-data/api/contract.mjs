...
const PATCH_FIELDS = {
  tracks: new Set(['name', 'artist', 'duration', 'bpm', 'key_signature', 'size', 'type', 'plays', 'likes', 'tags', 'lyrics', 'status', 'file_key', 'image_key']),
  playlists: new Set(['name', 'description', 'track_ids', 'start_color', 'end_color', 'image_key']),
  clients: new Set(['name', 'email', 'phone', 'avatar_key', 'company', 'status', 'last_active', 'tags']),
};
...
export function normalizeShareCreate(body = {}) {
  ...
  if (Boolean(trackId) === Boolean(playlistId)) {
    throw new Error('Share link must target exactly one track or playlist.');
  }
  return {
    id: requireUuid(body.id),
...
export function normalizeEntityCreate(entity, body = {}) {
  switch (entity) {
    case 'tracks':
      return normalizeTrackCreate(body);
    case 'playlists':
      return {
        id: requireUuid(body.id),
        name: requireText(body.name, 'name', 500),
    ...
    case 'share_links':
      return normalizeShareCreate(body);
    case 'activities':
      return {
        id: requireUuid(body.id),
    ...
    case 'promo_videos': {
      ...
      return {
        ...
        track_id: optionalUuid(body.track_id, 'track_id'),
        playlist_id: optionalUuid(body.playlist_id, 'playlist_id'),
        video_key: safeObjectKey(body.video_key, 'video_key'),
...
export function normalizePublicEvent(body = {}) {
  ...
  if (!new Set(['play', 'thumbs_up', 'thumbs_down', 'comment']).has(type)) {
    throw new Error('Public share event is invalid.');
  }
  const content = body.content == null ? null : String(body.content).trim();
  if (type === 'comment' && (!content || content.length > 4000)) {
    throw new Error('Comment content is required and must be 4000 characters or fewer.');
  }
  return {
    type,
    track_id: optionalUuid(body.track_id, 'track_id'),
