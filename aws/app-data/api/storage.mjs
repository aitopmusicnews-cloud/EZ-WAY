...
const CATEGORY = {
  ...
  'message-file': { prefix: 'messages/files', family: null, max: 100 * 1024 * 1024 },
...
export function normalizeUploadRequest(body = {}) {
  ...
  const contentType = String(body.contentType ?? body.content_type ?? 'application/octet-stream').trim().toLowerCase()
...
export async function presignRead(objectKey) {
  ...
  ensureBucket();
  const commandInput = { Bucket: bucket, Key: objectKey };
  if (String(objectKey).startsWith('messages/files/')) {
    commandInput.ResponseContentDisposition = 'attachment';
  }
  return getSignedUrl(s3, new GetObjectCommand(commandInput), { expiresIn: 30 * 60 });
}
